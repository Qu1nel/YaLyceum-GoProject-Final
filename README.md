...
*   **[ ]** Расширение интеграционных тестов для большего покрытия краевых случаев.
*   **[ ]** Более детальные юнит-тесты для некоторых компонентов.
*   **[ ]** Оптимизация производительности и масштабируемости (например, пул воркеров, более сложная балансировка).
...
### Юнит-тесты
Для запуска юнит-тестов для конкретного пакета (например, `internal/worker/service`):
```bash
go test -v -count=1 -cover ./internal/worker/service/...
```
Для запуска всех юнит-тестов в проекте (может потребовать предварительной генерации моков, если они отсутствуют):
```bash
# Убедитесь, что mockery установлен: go install github.com/vektra/mockery/v2@latest
# Сгенерировать все моки (если нужно): mockery --all --keeptree --case underscore
go test -v -count=1 -cover ./...
```
*(Флаг `-count=1` отключает кэширование тестов, `-cover` показывает покрытие)*.

### Интеграционные тесты
Интеграционные тесты находятся в директории `tests/integration/`. Они требуют запущенного Docker.
Для запуска:
```bash
# Сначала очистим кеш, чтобы гарантировать свежую сборку
go clean -cache
go clean -testcache
# Запускаем тесты с флагом -a для принудительной пересборки
go test -v -a ./tests/integration/...
```
`TestMain` в этом пакете автоматически поднимет все необходимые сервисы в тестовом окружении (PostgreSQL в testcontainers, Agent, Orchestrator, Worker как Go-процессы) и применит миграции перед запуском тестов.

## Демонстрация Работы

<!-- 
Здесь ты можешь вставить скриншоты или GIF-анимацию, показывающую работу приложения.
Например:
1. Скриншот главной страницы фронтенда (форма входа/регистрации).
   Описание: "Стартовая страница приложения, предлагающая войти или зарегистрироваться."
   ![Стартовая страница](путь/к/скриншоту1.png)
2. Скриншот интерфейса после входа с формой ввода выражения и списком задач.
   Описание: "Основной интерфейс после входа: форма для ввода математического выражения, список ранее отправленных задач с их статусами."
   ![Интерфейс калькулятора](путь/к/скриншоту2.png)
3. Скриншот с деталями вычисленной задачи (показывающий результат).
   Описание: "Просмотр деталей успешно вычисленной задачи с отображением результата."
   ![Детали задачи - успех](путь/к/скриншоту3.png)
4. Скриншот с деталями задачи, завершившейся ошибкой (например, деление на ноль).
   Описание: "Просмотр деталей задачи, завершившейся ошибкой, с отображением сообщения об ошибке."
   ![Детали задачи - ошибка](путь/к/скриншоту4.png)
5. (Опционально) Скриншот Swagger UI.
   Описание: "Интерфейс Swagger UI, демонстрирующий доступные эндпоинты API."
   ![Swagger UI](путь/к/скриншоту5.png)

Используй Markdown для вставки изображений: ![Описание](относительный/путь/к/изображению.png)
Изображения лучше всего поместить в директорию типа `.github/assets/` или `docs/images/` и ссылаться на них.
-->
