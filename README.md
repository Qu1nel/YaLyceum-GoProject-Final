...
*   **[X] Аутентификация:**
    ...
    *   **[X]** Реализация генерации JWT токенов при логине.
    *   **[X]** Реализация JWT Middleware для защиты эндпоинтов.
    *   **[X]** Извлечение `UserID` из токена и передача в контексте.
*   **[ ] API Agent:**
    *   **[X]** Реализация эндпоинта `POST /calculate` (прием выражения, запуск задачи).
...
*   **[X] gRPC:**
    ...
    *   **[X]** Реализация gRPC клиента в Agent для вызова Оркестратора.
    *   **[X]** Реализация gRPC сервера в Оркестраторе.
    *   **[X]** Определение `.proto` контракта Orchestrator <-> Worker.
    *   **[X]** Реализация gRPC клиента в Оркестраторе для вызова Воркеров.
    *   **[X]** Реализация gRPC сервера в Воркере.
*   **[ ] Оркестратор:**
    *   **[X]** Базовая структура сервиса (main.go, Dockerfile).
    *   **[X]** Получение задач от Агента по gRPC.
...
*   **[X] Воркер:**
    *   **[X]** Базовая структура сервиса (main.go, Dockerfile).
    *   **[X]** Получение подзадач от Оркестратора по gRPC.
    *   **[X]** Реализация вычисления арифметических операций.
    *   **[X]** Возврат результата Оркестратору.
*   **[-] Парсер Выражений:**
...
*   **[ ] Тестирование:**
    *   **[ ]** Юнит-тесты для ключевых компонентов (хешер, сервисы с моками, репозитории с моками.
