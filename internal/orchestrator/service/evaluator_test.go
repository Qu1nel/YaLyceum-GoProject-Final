// Файл: internal/orchestrator/service/evaluator_test.go
package service
...
// Теперь возвращает интерфейс Evaluator
func setupEvaluatorTest(t *testing.T) (Evaluator, *mocks.WorkerServiceClientMock) { // <--- ИЗМЕНЕН ВОЗВРАЩАЕМЫЙ ТИП
	logger := zap.NewNop()
	mockWorkerClient := mocks.NewWorkerServiceClientMock(t)
	evaluator := NewExpressionEvaluator(logger, mockWorkerClient) // NewExpressionEvaluator возвращает Evaluator
	return evaluator, mockWorkerClient
}

// --- Тесты для callWorker ---
// Метод callWorker является неэкспортируемым методом структуры ExpressionEvaluator.
// Чтобы его протестировать напрямую, нам нужен доступ к этой структуре.
// Если мы хотим сохранить setupEvaluatorTest возвращающим интерфейс,
// то callWorker нужно либо сделать экспортируемым (не рекомендуется для внутренних методов),
// либо тестировать его поведение косвенно через экспортируемый метод Evaluate.
//
// Однако, так как callWorker - это внутренняя деталь реализации *ExpressionEvaluator,
// и он не является частью интерфейса Evaluator, то в тестах, где мы получаем
// интерфейс Evaluator из setupEvaluatorTest, мы не сможем вызвать evaluator.callWorker.
//
// Для тестирования callWorker отдельно, нам нужно создать экземпляр *ExpressionEvaluator.
// Давай сделаем отдельный setup для этого или модифицируем setupEvaluatorTest,
// чтобы он возвращал и интерфейс, и конкретный тип для таких нужд.
//
// Либо, как мы сделали ранее, тесты для callWorker могут остаться, если мы
// в этих конкретных тестах приведем тип интерфейса Evaluator к *ExpressionEvaluator.

func TestExpressionEvaluator_callWorker_Success(t *testing.T) {
	// Для прямого теста callWorker нам нужен *ExpressionEvaluator
	logger := zap.NewNop()
	mockWorkerClient := mocks.NewWorkerServiceClientMock(t)
	evaluatorImpl := NewExpressionEvaluator(logger, mockWorkerClient).(*ExpressionEvaluator) // Приведение типа
	...
	result, err := evaluatorImpl.callWorker(ctx, opSymbol, opA, opB) // Используем evaluatorImpl
...
// --- Тесты для Evaluate (здесь используем интерфейс Evaluator из setupEvaluatorTest) ---
func TestExpressionEvaluator_Evaluate_IntegerNode(t *testing.T) {
	evaluator, _ := setupEvaluatorTest(t) // evaluator здесь типа Evaluator (интерфейс)
	node := &ast.IntegerNode{Value: 123}
	result, err := evaluator.Evaluate(context.Background(), node) // Вызываем метод интерфейса
	require.NoError(t, err)
