// Файл: internal/agent/service/task_test.go
package service
...
// Вспомогательная функция setupTaskServiceTest остается такой же
...
func TestTaskService_SubmitNewTask_gRPCError(t *testing.T) {
	...
	// 1. Проверяем, что сообщение нашей ошибки содержит текст оригинальной gRPC ошибки.
	// Теперь err.Error() будет "ошибка сервиса вычислений: rpc error: code = Unavailable desc = оркестратор недоступен"
	...
	// 2. Проверяем, что мы можем извлечь gRPC статус из обернутой ошибки.
	unwrappedErr := errors.Unwrap(err)
	require.NotNil(t, unwrappedErr, "Обернутая ошибка не должна быть nil") // Теперь это должно работать
...
func TestTaskService_GetUserTasks_Success(t *testing.T) {
    ...
    userID := uuid.New().String()
    now := time.Now()
    // Для gRPC время часто передается как строка RFC3339Nano
    ...
    // Сравниваем время с учетом возможной потери точности при парсинге
...
func TestTaskService_GetTaskDetails_NotFound(t *testing.T) {
    ...
    userID := uuid.New().String()
    taskID := uuid.New().String()
    // Ошибка, которую вернет мок Оркестратора
    ...
    require.Error(t, err)
    // Проверяем, что TaskService обернул ее в нашу ошибку ErrTaskNotFound
    assert.ErrorIs(t, err, ErrTaskNotFound, "Ошибка должна быть ErrTaskNotFound")
    // И что сообщение содержит оригинальное сообщение от gRPC
